...
router.get('/', function(request, response){
  ...
  pg.connect(connectionString, function(err, client, done){
    if(err){
     ...
     'JOIN userCupboard ON users.username = userCupboard.username JOIN products '+
    'ON products.product_name=userCupboard.product_name WHERE username = $1', [user.username], function(err, result){
       if(err){
         console.log(err);
         response.sendStatus(500);
       }else{
